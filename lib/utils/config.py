        ...
        for key, value in other.__dict__.items():
            if key not in self.__dict__:
                ...
                if current_value != value:
                    print(f"  Overwriting {key} from {current_value} to {value}.")
                    setattr(new, key, value)
        return new
            ...
            if key in self.__dict__:
                ...
                if isinstance(current_value, Config):
                    if isinstance(value, dict):
                        ...
                        raise ValueError(f"Cannot overwrite the category {current_value}.")
                else:
                    print(f"  Overwriting '{key}' from '{current_value}' to '{value}'.")
                ...
                for dict_value in self.__dict__.values():
                    if isinstance(dict_value, Config):
                        try:
                            ...
                            updated_any = True
                if not updated_any:
                    raise KeyError(f"'{key}' is not a key in Config")
