...
from lib.datasets.imagenet_vid_mmdet.mmtracking.utils.logger import get_root_logger


# from third_party.mmtracking.mmtrack.core import eval_mot
...
class CocoVideoDataset(CocoDataset):
    ...
    Args:
        load_as_video (bool): If True, using COCOVID class to load dataset,
            otherwise, using COCO class. Default: True.logger.py
    ...
    def ref_img_sampling(self,
        ...
        if (not self.load_as_video) or img_info.get('frame_id', -1) < 0 \
            ...
            if method == 'uniform':
                ...
                # fixme by AmirH: ensure to return num_ref_imgs frames
                if len(valid_ids) == 0:
                    valid_ids.append(img_id)

                for i in range(len(valid_ids), num_ref_imgs):
                    valid_ids.append(valid_ids[-1])
